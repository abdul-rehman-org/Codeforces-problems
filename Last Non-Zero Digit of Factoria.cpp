...
// Extended Euclidean algorithm for modular inverse (mod not necessarily prime)
...
// Precompute prefix product array for numbers up to pe that are not divisible by p
...
// Compute n! with all factors of p removed, modulo pe
// f is the precomputed array of size pe
...
// Exponent of prime p in n! (Legendre's formula) - safe version
...
int last_digit(long long n, int b) {
    ...
    // Prime factorisation of b into (p, exponent, p^exponent)
    ...
    // Compute v_p(n!) for each prime and determine Z = min(v_p / e)
    ...
    // Compute M_i = (n! / b^Z) mod p_i^{e_i} for each prime power
    ...
    for (size_t idx = 0; idx < factors.size(); ++idx) {
        ...
        long long A = fact_mod_free(n, p, pe, f);   // p-free part mod pe
    ...
    // Combine remainders using Chinese Remainder Theorem
    long long M = 1;
